...
> [MT] In der Einleitung zu diesem Bereich wird ja schon erwähnt, dass das meist nur bei Open-Source-Software möglich ist. Daher ist eine eigene Kennzeichnung, nicht unbedingt notwendig, denke ich.

Die Dokumentation einer Software ist essentiell, um Anwendungsentwickler das benötigte Verständnis zu bieten, das die ggf. Erweiterung einer Software ermöglicht.

Nur durch eine ausreichende Dokumentation wird eine ausreichend große Community von potenziellen Entwicklern angesprochen und zum Anderen ein umfassendes Verständnis der Intention der Software, der Ausgereiftheit und des aktuellen Entwicklungsstandes möglich.

> [MT] Ich denke nicht, dass der Umfang einer Dokumentation der alleinige Grund für eine große Entwicklergemeinschaft ist.
Stattdessen einfach nur: Eine umfangreiche Dokumentation bietet ein umfassendes Verständnis über die Intention, der Ausgereiftheit und des aktuellen Entwicklungsstandes der Software.

Man unterscheidet mehrere Dokumentationsbestandteile: die `Dokumentation des Quellcodes`, d.h. von Klassen oder einzelnen Methoden, die `Dokumentation des Build-Prozesses`, d.h. wie die Software aus dem Quellcode erzeugt wird, eine `Entwicklerdokumentation` mit Beispielen Nutzungsbeispielen, und eine `Dokumentation dessen, wie die Software getestet` wurde bzw. welche Testfälle von der Software berücksichtigt wurden.

> [MT] `Dokumentation dessen, wie die Software getestet` -> 'Dokumentation der Softwaretests'?
...
> [MT] Da CI hier schon genannt ist, habe ich die Erläuterung hier er verschoben. Man könnte aber auch überlegen das als eigene Frage in diesem Bereich aufzugreifen. Oder, wie ganz unten angemerkt, allen Aspekten in der Entwcklerperspektive voran zu stellen.

*Continuous Integration* (CI, Kontinuierliche Integration) bezeichnet die  fortlaufende Zusammenfügung der Einzelkomponenten einer Anwendung. Dafür werden Routinen erstellt, die bestimmte Aufgaben durchführen und einen Ergebnisbericht liefert. So kann beispielsweise automatisiert die Erstellung einer Quellcodedokumentation, die Durchführung eines Softwaretests oder die Erzeugung einer ausführbaren Datei aus dem Quellcode (Releasefile, exe-Datei) angestoßen werden. Dabei werden diese Routinen meist nach jeder Änderung des Quellcodes erneut ausgeführt, wodurch alle Programmkomponenten aktuell bleiben.
...
> [MT] der Zweite Punkt (Optik) wird eigentlich schon in 'Interface', insbesondere 'Orientiert sich die Menüführung an bestimmten Vorbildern?' oben behandelt und kann hier weg.
